...
-- Add manage_subscriptions permission
INSERT INTO `Permission` (`id`, `name`, `description`) VALUES 
(UUID(), 'manage_subscriptions', 'Can manage subscription plans and user subscriptions');

-- Assign manage_subscriptions permission to admin role
INSERT INTO `RolePermission` (`roleId`, `permissionId`) 
SELECT 
    (SELECT `id` FROM `Role` WHERE `name` = 'admin'),
    (SELECT `id` FROM `Permission` WHERE `name` = 'manage_subscriptions');